...
async function trackUserAction(userId, actionKey) {
  await fetch('https://incenta.dev/api/v1/incentives/events', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${process.env.INCENTA_API_KEY}`,
      'Content-Type': 'application/json',
...
```bash
curl https://incenta.dev/api/v1/incentives/leaderboard \
...
```bash
curl https://incenta.dev/api/v1/incentives/marketplace/verify/user_123 \
  -H "Authorization: Bearer rk_xxxxx"
```
...
```bash
curl -X POST https://incenta.dev/api/v1/incentives/marketplace/redeem \
  -H "Authorization: Bearer rk_xxxxx" \
...
async function checkReferralHealth(campaignId) {
  const res = await fetch(
    `https://incenta.dev/api/v1/stats?campaignId=${campaignId}`,
    { headers: { Authorization: `Bearer ${process.env.INCENTA_API_KEY}` } }
  )
